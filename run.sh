...
PGPASSWORD=${POSTGRES_TESTUSER_PASSWORD?} pg_prove ${POSTGRES_HOST:+ -h $POSTGRES_HOST} ${POSTGRES_PORT:+ -p $POSTGRES_PORT} ${POSTGRES_DBNAME:+ -d $POSTGRES_DBNAME} ${POSTGRES_TESTUSER:+ -U $POSTGRES_TESTUSER} ${PGPROVE_ARGS} /tests/*.sql
